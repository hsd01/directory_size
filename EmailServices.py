...
def email_with_attachhment(username, path2key, mail_to, path):
    ...
    mail_subject = "Disk Utilisation report for "+socket.gethostname()
    ...
    filename = "/{}Output D-{}-{}-{} T.csv".format(socket.gethostname(), latest_date[1], latest_date[2], latest_date[0])
...
def disk_usage(path, output_path, pc_name='', SAVE = True):
    #path = 'C:/'
    output = {}
    os.chdir(path)
    folder = os.listdir()
    ...
    #time_ = str(current_time)
    #time_ = current_time[11:16]
    filename = "/{}Output D-{}-{}-{} T.csv".format(pc_name, latest_date[1], latest_date[2], latest_date[0])
